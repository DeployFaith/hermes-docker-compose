...
read -r -p "  OpenRouter API key (recommended, openrouter.ai/keys): " OPENROUTER_KEY
...
# Nous
read -r -p "  Nous Research API key (nous.ai): " NOUS_KEY
if [[ -n "$NOUS_KEY" ]]; then
...
HAS_KEY=false
for var in OPENROUTER_KEY ANTHROPIC_KEY GOOGLE_KEY OLLAMA_KEY NOUS_KEY; do
    if [[ -n "${!var:-}" ]]; then HAS_KEY=true; break; fi
done

# Also check if keys already existed in .env from a prior run
if ! $HAS_KEY; then
    for line in OPENROUTER_API_KEY ANTHROPIC_API_KEY GOOGLE_API_KEY OLLAMA_API_KEY NOUS_API_KEY; do
